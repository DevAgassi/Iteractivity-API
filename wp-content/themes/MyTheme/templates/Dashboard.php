				...
				<section class="p-10 space-y-12 bg-[var(--color-background)] min-h-screen">
					...
					<div class="space-y-8">
						...
						<div
							...
							{# Solid #}
							<div class="space-y-4">
								<h3 class="text-xl font-semibold uppercase tracking-wider text-[var(--color-text)]">1. Solid Colors (Default)</h3>
								<div class="flex flex-wrap gap-4 p-6 bg-white rounded-xl shadow-sm">
									<a href="#" class="inline-flex items-center text-white bg-primary box-border border border-transparent hover:bg-brand-strong focus:ring-4 focus:ring-brand-medium shadow-xs font-medium leading-5 rounded-base text-sm px-2 focus:outline-none">
										Read more
									</a>
									{{ ui_button({ label: 'Success Solid' }) }}
									{{ ui_button({ intent: 'secondary', label: 'Primary Solid' }) }}
									{{ ui_button({ intent: 'error', label: 'Error Solid' }) }}
									{{ ui_button({ intent: 'success', label: 'Error Solid' }) }}
								</div>
							</div>
							...
							<div class="space-y-4">
								...
								<div class="flex flex-wrap gap-4 p-6 bg-white rounded-xl shadow-sm">
									{{ ui_button({ label: 'Primary Outlined', variant: 'outlined', }) }}
									{{ ui_button({ label: 'Warning Outlined', variant: 'outlined', intent: 'error' }) }}
								</div>
							</div>
						...
						<div class="space-y-4">
							...
							<div class="gap-4 p-6 bg-white rounded-xl shadow-sm">
								...
								color: 'secondary',
								icon_after: '<svg class="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path d="M14 5l7 7m0 0l-7 7m7-7H3"></path></svg>' 
							}) }}
							</div>
						</div>
					</div>
					...
					<header class="border-b border-[var(--color-border)] pb-6">
						...
						<p class="text-[var(--color-text)] mt-2">Тестування всіх варіацій, кольорів та станів компонента.</p>
					</header>
					...
					<div class="grid grid-cols-1 md:grid-cols-2 gap-8">
						<div class="space-y-4">
							<h3 class="text-xl font-semibold text-[var(--color-text)] uppercase tracking-wider">2. Outlined Variant</h3>
						...
						<div class="space-y-4">
							<h3 class="text-xl font-semibold text-[var(--color-text)] uppercase tracking-wider">3. Ghost Variant</h3>
							<div class="flex flex-wrap gap-4 p-6 bg-white rounded-xl shadow-sm border border-gray-100">
					...
					<div class="space-y-4">
						<h3 class="text-xl font-semibold text-[var(--color-text)] uppercase tracking-wider">4. Reverse Mode (On Dark Background)</h3>
					...
					<div class="space-y-4">
						<h3 class="text-xl font-semibold text-[var(--color-text)] uppercase tracking-wider">5. States & Types</h3>
					...
					<div class="space-y-4">
						<h3 class="text-xl font-semibold text-[var(--color-text)] uppercase tracking-wider">6. Brand Specific (SK Blue)</h3>
						<div class="gap-4 p-6 bg-gray-50 border-2 border-dashed border-gray-200 rounded-xl">
							{{ ui_link({ label: 'SK Primary', color: 'primary' }) }}
							{{ ui_link({ label: 'SK Outlined', color: 'primary', variant: 'outlined' }) }}
							{{ ui_link({ label: 'SK Ghost', color: 'primary', variant: 'ghost', type: 'link' }) }}
							{{ ui_link({ label: 'SK No BG', color: 'primary', variant: 'ghost', type: 'link', no_bg: true }) }}
						</div>
					</div>

					{# 6. СЕКЦІЯ: РОЗМІРИ (SIZES) #}
					<div class="space-y-4">
						<h3 class="text-xl font-semibold text-[var(--color-text)] uppercase tracking-wider">7. Button Sizes</h3>
