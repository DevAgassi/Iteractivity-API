import { store, getContext } from "@wordpress/interactivity";
  ...
  function calculate_dough(context) {
    ...
    let { unitFlourKg, unitWaterL, unitSaltKg, unitOilL, unitYeastKg } =
      context;
    ...
    ingredients.flour = Math.ceil(
      (calculations.numberOf * calculations.weightOf) / bakersBase,
    );

    const flour = convertToMetricIfNeeded(
      ingredients.flour,
      1000,
      "unitFlourKg",
    );
    ...
    ingredients.water = Math.ceil(
      ((calculations.numberOf * calculations.weightOf) / bakersBase / 100) *
        calculations.hydrationOf,
    );
    ...
    const water = convertToMetricIfNeeded(
      ingredients.water,
      1000,
      "unitWaterL",
    );
    ...
    ingredients.salt = Math.ceil(
      (Math.ceil((calculations.numberOf * calculations.weightOf) / bakersBase) /
        100) *
        3,
    );
    ...
    ingredients.yeast = Math.ceil(
      ((calculations.weightOf * calculations.numberOf) / 100) * 0.115,
    );

    const yeast = convertToMetricIfNeeded(
      ingredients.yeast,
      1000,
      "unitYeastKg",
    );
  ...
  store("calculator", {
    actions: {
      pizzas: (event) => {
        ...
        console.log("Pizzas changed:", context);
