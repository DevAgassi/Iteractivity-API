...
document.addEventListener('DOMContentLoaded', () => {
    ...
    menuItems.forEach(item => {
        ...
        const resetSubLevels = () => {
            ...
            subLinks.forEach(l => l.classList.remove('bg-white', 'text-blue-600', 'shadow-sm'));
        };

        const showMenu = () => {
            clearTimeout(closeTimeout);
            mega.classList.remove('invisible', 'opacity-0', 'pointer-events-none', '-translate-y-1');
            mega.classList.add('visible', 'opacity-100', 'pointer-events-auto', 'translate-y-0');
        ...
        const hideMenu = () => {
            closeTimeout = setTimeout(() => {
                mega.classList.remove('visible', 'opacity-100', 'pointer-events-auto', 'translate-y-0');
                mega.classList.add('invisible', 'opacity-0', 'pointer-events-none', '-translate-y-1');
                resetSubLevels();
